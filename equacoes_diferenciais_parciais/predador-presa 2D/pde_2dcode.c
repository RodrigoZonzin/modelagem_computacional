...
inline float g_diffusion(float w){
    return (1.0 - w) < 0.0? 0.0: (1.0 - w);
}
...
int main(){    
    ...
    float axis_min = 0.0, axis_max = 1.0, axis_delta = 0.1;
    ...
    for (int j = 0; j < y_size; j++) {
        ...
        for (int i = 0; i < x_size; i++) {
            if ((i >= (h_field->x_axis.size/2.0)) && (i <= (h_field->x_axis.size/2.0 + 1)) &&
                (j >= (h_field->y_axis.size/2.0)) && (j <= (h_field->y_axis.size/2.0 + 1))
            ) {
                set_current_value(h_field, i, j, 1.0);
                set_current_value(p_field, i, j, 0.0);
            }
            else {
                set_current_value(h_field, i, j, 0.0);
                //set_current_value(p_field, i, j, 0.2);
            }

            if ((i >= 0 && i < 5) &&
                (j >= 0 && j < 5)
            ) {
                set_current_value(p_field, i, j, 1.0);
            }
        }
    }
    ...
    for(int step = 1; step <= nsteps; ++step) {
        ...
        for (int j = 0; j < y_size; j++) {
            ...
            for (int i = 0; i < x_size; i++) {
                ...
                //printf("%f, %f, %f\n", w, h_field->x_axis.values[i], h_field->y_axis.values[i]);
                
                //delHdelt
                float h_next = rh*h *g(get_sum(fields,i,j)) + Dh*local_average(fields, h_field, i, j);
                //delPdelt
                float p_next = (a*h*p*g(get_sum(fields,i,j))) + Dp*local_average(fields, p_field, i, j);
